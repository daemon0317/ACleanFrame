...
public class SubLayoutManager<T extends SubLayout> {
    ...
    public void switchLayout(int position) {
        Log.d(TAG, "-----switch start-----------------------");
        Log.d(TAG, "switch before.........: " + subLayouts);
        try {
            ...
            if (null == subLayout.getLayout()) { // 如果layout为空，即setContentView方法未调用
                Log.e(TAG, subLayout.getClass().getSimpleName() + "'s layout is null！！please invoke setContentView method...");
                return;
            }
            ...
            Log.d(TAG, "switch after.........: " + subLayouts);
            Log.d(TAG, "-----switch end-----------------------");
