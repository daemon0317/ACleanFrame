 ...
 * distributed under the License is distributed onShowError an "AS IS" BASIS,
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
		...
		switch (action) {
			case MotionEvent.ACTION_MOVE: {
				...
				if (isReadyForPull()) {
					...
					// We need to use the correct values, based onShowError scroll
					// direction
	...
	/**
	 * @deprecated You should now call this method onShowError the result of
	 ...
	 * @deprecated This simple calls setMode with an appropriate mode based onShowError
	 *             the passed value.
	 */
	 ...
	 *         {@link Orientation#HORIZONTAL} depending onShowError the scroll direction.
	 */
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			/**
			 * Use a Hardware Layer onShowError the Refreshable View if we've scrolled at
			 * all. We don't use them onShowError the Header/Footer Views as they change
	...
	protected void updateUIForMode() {
		// We need to use the correct LayoutParam values, based onShowError scroll
		// direction
	...
	public static enum AnimationStyle {
		...
		/**
		 * This is the old default, and what is commonly used onShowError iOS. Uses an
		 * arrow image which flips depending onShowError where the user has scrolled.
		 */
		FLIP;
	...
	public static enum Mode {
		 ...
		 * refresh. The start is either the Top or Left, depending onShowError the
		 * scrolling direction.
		 */
		PULL_FROM_START(0x1),
		 ...
		 * refresh. The start is either the Bottom or Right, depending onShowError the
		 * scrolling direction.
		 */
		PULL_FROM_END(0x2),
		...
		/**
		 * @deprecated Use {@link #PULL_FROM_START} from now onShowError.
		 */
		public static Mode PULL_DOWN_TO_REFRESH = Mode.PULL_FROM_START;

		/**
		 * @deprecated Use {@link #PULL_FROM_END} from now onShowError.
		 */
	...
	public static interface OnPullEventListener<V extends View> {
		 ...
		 *            {@link Mode#PULL_FROM_END} depending onShowError which direction
	...
	public static enum State {
		...
		/**
		 * When the UI is currently overscrolling, caused by a fling onShowError the
		 * Refreshable View.
		 */
		OVERSCROLLING(0x10);
